    ...
    , ui(new Ui::WeatherWidget)
{
    ...
    const QList<PostCodeInfo> &allpcode = PostCode::getInstance().getALLPostCode();
    QStringList allregion;
    for(int i=0;i<allpcode.size();i++){
        allregion.append(allpcode.at(i).region);
    }

    ui->comboBoxr->addItems(allregion);
    ui->comboBoxr->setCurrentText("成都");

    // 设置可编辑
    ui->comboBoxr->setEditable(true);
    // 自动补全实例的构建
    QCompleter *compr = new QCompleter(allregion,ui->comboBoxr);
    // 设置匹配模式，只要包含就显示
    compr->setFilterMode(Qt::MatchContains);
    // 设置大小写区分，不区大小写
    compr->setCaseSensitivity(Qt::CaseInsensitive);
    // 设置向用户提供补全的方式
    compr->setCompletionMode(QCompleter::PopupCompletion);
    // 装载补全实例
    ui->comboBoxr->setCompleter(compr);
...
{
    PublicIP::getInstance().requestPubliceIP(); // 获取公网IP
}
...
void WeatherWidget::on_pushButtonr_clicked()
{
    PostCodeInfo pcode;
    if(PostCode::getInstance().getPostCodeFromRegion(pcode,ui->comboBoxr->currentText())){
        QString str;
        str += " 邮编:";
        str += " " + pcode.provinceAbbrevate;
        str += " " + pcode.province;
        str += " " + pcode.region;
        str += " " + QString::number(pcode.regionCode);
        str += " " + QString::number(pcode.postCode);
        ui->labelr->setText(str);
        qdebug<<str;
    }
}
