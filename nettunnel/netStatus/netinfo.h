...
/*
   ...
   使用awk或者sed命令进一步处理：你可以使用awk或者sed命令来处理和过滤数据，例如：cat /proc/net/dev | awk '{print
      $1, $2}'可以只打印出网络设备名称和接收流量的统计值.
   ...
   face |bytes    packets errs drop fifo frame compressed multicast|bytes
    ...
    lo: 52372248   20108    0    0    0     0          0         0 52372248
         20108    0    0    0     0       0          0
   eth0: 17748776    4871    0    0    0     0          0       507   280619
         3192    0    0    0     0       0          0
...
// cat /proc/net/netstat |  awk '(f==0) {name=$1; i=2; while ( i<=NF) {n[i] =
// $i; i++ }; f=1; next} (f==1){ i=2; while ( i<=NF){ printf "%s%s = %d\n",
// name, n[i], $i; i++}; f=0} '
...
signals:
    ...
    void sig_netInfo(QString& rdata);
};
