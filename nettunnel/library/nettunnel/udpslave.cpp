    ...
    : QObject{parent},udpsocket(nullptr)
{
    ...
    connect(timer,&QTimer::timeout,this,[=](){
        ...
        if(m_isClient){
            QByteArray senddata;
            senddata.resize(2);
            senddata[0] = (quint8)0xAB;
            senddata[1] = (quint8)0xC7;
            udpsocket->writeDatagram(senddata,m_serverAddr,m_serverPort);
        }
        if(m_isServer){
            QByteArray senddata;
            senddata.resize(8);
            senddata[0]= (quint8)0xBA;
            senddata[1] = (quint8)0X7C;
            srand(QDateTime::currentMSecsSinceEpoch());
            quint8 dekey = (quint8)(rand()%256);//0x1F;
            quint8 lenght = 8;
            senddata[3] = lenght;
    ...
    connect(timer1,&QTimer::timeout,this,[=](){
        for(int i=0;i<m_serverIPNodeList.size();i++){
            if(5000 < m_serverIPNodeList[i].datetime.msecsTo(QDateTime::currentDateTime())){
...
bool UdpSlave::open(QHostAddress serverAddr,quint16 serverPort)
{
    if(udpsocket == nullptr){
        udpsocket = new QUdpSocket(this);
        if(false == udpsocket->bind(QHostAddress::Any)){
            ...
            qdebug<<QString("open success,%1:%2").arg(udpsocket->localAddress().toString()).arg(udpsocket->localPort());
            ...
            connect(udpsocket,&QUdpSocket::readyRead,this,[&](){
                ...
                // qdebug << str;
                //qdebug << data;
                if (false == parseServerIP(data)){
                    emit recvUdpData(data,addr,port);
                }
        ...
        qdebug<<QString("have opened,%1:%2").arg(udpsocket->localAddress().toString()).arg(udpsocket->localPort());
        return false;
    }
}
...
qint64 UdpSlave::sendData(const QByteArray &data, const QHostAddress &addr, const quint16 &port)
{
    qint64 ret = udpsocket->writeDatagram(data,addr,port);
    if(ret < 0){
        qdebug <<udpsocket->errorString();
    }
    return ret;
