...
public:
    ...
    void start(const string& dir,
               unsigned int  flushMs,
               unsigned int  fileLogMaxSize);
    void writeLog(LOG_TYPE_ENUM level,
                  const char   *function,
                  const char   *file,
                  const int     line,
                  const char   *data);
...
#define DEBUG_LOG(__format, ...) WRITE_LOG(LOG_TYPE_ENUM_DEBUG, \
                                           __format,            \
                                           ## __VA_ARGS__)
#define WARRING_LOG(__format, ...) WRITE_LOG(LOG_TYPE_ENUM_WARRING, \
                                             __format,              \
                                             ## __VA_ARGS__)
#define CRITICAL_LOG(__format, ...) WRITE_LOG(LOG_TYPE_ENUM_CRITICAL, \
                                              __format,               \
                                              ## __VA_ARGS__)
#define FATAL_LOG(__format, ...) WRITE_LOG(LOG_TYPE_ENUM_FATAL, \
                                           __format,            \
                                           ## __VA_ARGS__)
#define INFO_LOG(__format, ...) WRITE_LOG(LOG_TYPE_ENUM_INFO, \
                                          __format,           \
                                          ## __VA_ARGS__)
