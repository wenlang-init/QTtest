...
{
    if ((len <= 0) || (inVec.isEmpty()) || (outVec.isEmpty())) return false;
    ...
    for (int L = 1; L <= r; L++) {
        ...
        for (int b = 0; b < B; b++) {
            ...
            for (int n = 0; n < N / 2; n++) {
                ...
                pVec[index].rl = X[index].rl +
                                 (Weights[n * W].rl * X[dist].rl -
                                  Weights[n * W].im * X[dist].im); // Fe + W*Fo
                pVec[index].im = X[index].im + Weights[n * W].im * X[dist].rl +
                                 Weights[n * W].rl * X[dist].im;
            }

            for (int n = N / 2; n < N; n++) {
                int index = n + mid;
                pVec[index].rl = X[index - distance].rl + Weights[n * W].rl *
                                 X[index].rl - Weights[n * W].im * X[index].im; //
                                                                                // Fe
                                                                                // -
                                                                                // W*Fo
                pVec[index].im = X[index - distance].im + Weights[n * W].im *
                                 X[index].rl + Weights[n * W].rl * X[index].im;
            }
        }
...
bool fft::ifft(const Complex inVec[], const int len, Complex outVec[])
{
    ...
    for (int L = r; L >= 1; L--) {
        ...
        // sprintf(msg + 6, "B %d, N %d, W %d, distance %d, L %d", B, N, W,
        // distance, L);
        ...
        for (int b = 0; b < B; b++) {
            for (int n = 0; n < N / 2; n++) {
                ...
                // sprintf(msg + 6, "%d, %d: %lf, %lf", n + 1, index,
                // X2_rl[index], X2_im[index]);
                // OutputDebugStringA(msg);
            }
            ...
            for (int n = N / 2; n < N; n++) {
                ...
                X2_im[index] = (X_im[index] - X_im[index - distance]) / 2;
                double square = W_rl[n * W] * W_rl[n * W] + W_im[n * W] *
                                W_im[n * W];                               // c^2+d^2
                double part1 = X2_rl[index] * W_rl[n * W] + X2_im[index] *
                               W_im[n * W];                                // a*c+b*d
                double part2 = X2_im[index] * W_rl[n * W] - X2_rl[index] *
                               W_im[n * W];                                // b*c-a*d
    ...
    for (int i = 0; i < len; i++) {
        ...
        // sprintf(msg + 6, "X_rl[i]: %lf, %lf,  index: %d", out_rl[i],
        // out_im[i], index);
        // OutputDebugStringA(msg);
    }
...
int fft::calMeanData(QList<double>data, QList<double>& retData, double fs)
{
    ...
    for (int i = 0; i < index; i++) cntNum += data[i];

    double mean = cntNum / static_cast<double>(index);

    for (int i = 0; i < index; i++) retData.push_back(data[i] - mean);
    return m;
}
...
void fft::timeDomainToFreDomain(const CurveData& timeData,
                                ...
                                int              fs)
{
    ...
    for (int index = 0; index < timeData.valVec.size(); index++) valData.append(timeData.valVec.at(
                                                                                    index));
    ...
    {
        double tmp = 2 *
                     sqrt((outVec[i].rl * outVec[i].rl) +
                          (outVec[i].im * outVec[i].im)) /
                     size;
        ...
        frequencyDomainData.keyVec.push_back((static_cast<double>(i) * 2.0 /
                                              size));
    }
}
