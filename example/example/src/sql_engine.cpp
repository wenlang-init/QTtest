...
{
    if (_sql_db == nullptr) {
        connectionName = txt.split("/").last() + "." + QString::number(
            QDateTime::currentDateTime().toMSecsSinceEpoch());

        // QIBASE, QSQLITE, QMIMER, QOCI, QODBC, QPSQL
        // QSQLITE3, QSQLITE2, QMYSQL, QDB2
        ...
        // _sql_db = new
        // QSqlDatabase(QSqlDatabase::addDatabase("QODBC",connectionName));
        // _sql_db = new
        // QSqlDatabase(QSqlDatabase::addDatabase("QPSQL",connectionName));
        // _sql_db = new
        // QSqlDatabase(QSqlDatabase::addDatabase("QMYSQL",connectionName));
    ...
    // _sql_db->setConnectOptions("QSQLITE_BUSY_TIMEOUT=1");
...
bool Sql_Engine::createTable(const QString& tablename,
...
bool Sql_Engine::addcolumn(const QString& tablename,
                           const QString& columnname,
                           const QString& columntype)
{
    if (isOpen() == false) return false;

    QString cmd = QString("alter table %1 add column if not exists %2 %3")
                  .arg(tablename)
                  .arg(columnname)
                  .arg(columntype);
    QSqlQuery query(*_sql_db);
    ...
    if (false == query.exec(cmd)) {
        qdebug << QString("%1, err=%2")
            .arg(cmd)
            .arg(_sql_db->lastError().text());
        return false;
    }
    return true;
}

bool Sql_Engine::delcolumn(const QString& tablename, const QString& columnname)
{
    if (isOpen() == false) return false;

    QString cmd = QString("alter table %1 drop column %2")
                  .arg(tablename)
                  .arg(columnname);
    QSqlQuery query(*_sql_db);
...
bool Sql_Engine::queryTabbleInfo(const QString& tablename,
                                 QMap<QString, QString>& tableinfo)
{
    ...
    else
    {
        ...
        // QVariant::int "pk" "0"
        // QSqlRecord qsqlrd =
        // query_data.record();qdebug<<qsqlrd.count();for(int
        // i=0;i<qsqlrd.count();i++)
        // qdebug<<qsqlrd.field(i)<<qsqlrd.fieldName(i)<<qsqlrd.value(i).toString();
        while (query_data.next()) {
            // for(int i=0;i<query_data.size();i++) qdebug <<
            // query_data.value(i).toString();
            tableinfo[query_data.value("name").toString()] = query_data.value(
                "type").toString();
        }
    }
