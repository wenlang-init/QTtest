...
ComboBox {
    ...
    signal modelNodeClicked(var text)
    ...
    contentItem: TextInput {
        ...
        selectByMouse: true
        readOnly: !comboBox.filterAble
        onTextEdited:{
            comboBox.dataFilter(text)
        }
    }
    indicator: Canvas {
        id: canvas
        x: comboBox.width - width - comboBox.indicatorPadding
        ...
        onPaint: {
            ...
            context.closePath();
            context.fillStyle = comboBox.textColor;
            context.fill();
        }
    }
        ...
        contentItem: ListView {
            ...
            delegate: Item{
                ...
                Row{
                    ...
                    Text {
                        height:30
                        text: filterListView.model.modelData
                ...
                MouseArea{
                    ...
                    hoverEnabled: true
                    onEntered: {
                        filterListView.currentIndex = filterListView.model.index
                    }

                    onClicked: (mouse)=>{
                        //mouse.accepted = true
                        var index = comboBox.find(filterListView.model.modelData)
                        filterListView.currentIndex = filterListView.model.index
                        popupFilter.close()
                        console.log(filterListView.model.modelData)
                        comboBox.modelNodeClicked(filterListView.model.modelData)
                    }
                }
            }
        }
