...
void TMView::drawBackground(QPainter *painter, const QRectF& rect)
{
    ...
    // painter->drawImage(pixmapbackground.rect(),QImage(":/images/Background.png"));
...
void TMView::wheelEvent(QWheelEvent *e)
{
    ...
    for (int i = 0; i < items.size(); i++) {
        items[i]->setScale(
            e->angleDelta().ry() >
            0 ? items[i]->scale() * 1.1 : items[i]->scale() * 0.9);
    }

    {
        ...
        qdebug << cursorPoint << scenePos << transform().m11() <<
            transform().m12()
...
void TMView::resizeEvent(QResizeEvent *resizeEvt)
{
    ...
    // 等比缩放
    fitInView(pixmapbackground.rect(), Qt::AspectRatioMode::KeepAspectRatio);

    // fitInView(pixmapbackground.rect(),
    // Qt::AspectRatioMode::IgnoreAspectRatio); // 完全填充
    // fitInView(pixmapbackground.rect(),
    // Qt::AspectRatioMode::KeepAspectRatioByExpanding); // 等比填充
