...
public:
    ...
    // static Sql_Engine& instance(){static Sql_Engine sqlengine;return
    // sqlengine;}
    virtual ~Sql_Engine();
    ...
    // 添加列
    bool addcolumn(const QString& tablename,
                   const QString& columnname,
                   const QString& columntype);

    // 删除列
    bool delcolumn(const QString& tablename,
                   const QString& columnname);
    ...
    // 查询表信息;列名,类型
    bool queryTabbleInfo(const QString& tablename,
...
protected:
    ...
    // 升序
    void sqrtparams(QList<QString>& params,
                    QList<int>    & index);
