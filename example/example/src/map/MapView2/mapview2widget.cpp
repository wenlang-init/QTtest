#include "mapview2widget.h"
#include "src/map/public/bingformula.h"
#include "ui_mapview2widget.h"
...
// using MapGraphicsView = MapGraphicsView2;
static MapGraphicsView2 *g_view = nullptr; // 当前视图

MapView2Widget::MapView2Widget(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::MapView2Widget)
{
    ui->setupUi(this);
    ...
    this->setWindowTitle(QString("QT加载显示离线瓦片地图示例（绝对像素坐标）--V%1").arg(1.0));

    g_view = ui->graphicsView;
    qRegisterMetaType<ImageInfo>("ImageInfo");

    connect(ui->graphicsView,
            &MapGraphicsView2::zoom,
            this,
            &MapView2Widget::zoom);
    connect(ui->graphicsView, &MapGraphicsView2::showRect, this,
            &MapView2Widget::showRect);
    connect(ui->graphicsView, &MapGraphicsView2::mousePos, this,
            &MapView2Widget::mousePos);
}

MapView2Widget::~MapView2Widget()
{
    delete ui;
}

void MapView2Widget::on_but_open_clicked()
{
 ...
 * @return
 */
QVector<int>MapView2Widget::getDir(const QString& path)
{
    QVector<int> vector;
    QDir dir(path);
 ...
 * @return
 */
QVector<int>MapView2Widget::getFile(const QString& path)
{
    QVector<int> vector;
    QDir dir(path);
 ...
 * @param level   地图层级Z
 */
void MapView2Widget::getTitles(int level)
{
    ...
    QString strRect = QString("[%1，%2] - [%3，%4] / ").arg(lt.x()).arg(lt.y()).arg(
        rd.x()).arg(rd.y());
 ...
 * @brief 加载所有瓦片图
 */
void MapView2Widget::loadImages()
{
    ...
    {
        QString path = root + QString("%1/%2.%3").arg(tile.x()).arg(tile.y()).arg(
            format);
        info.url = path;
        info.x = tile.x();
        info.y = tile.y();
...
void MapView2Widget::zoom(bool flag)
{
    // 判断之前的线程是否执行完成
 ...
 * @param rect
 */
void MapView2Widget::showRect(QRect rect)
{
    QString str = QString("[%1，%2] - [%3，%4]").arg(rect.x()).arg(rect.y()).arg(
        rect.left()).arg(rect.bottom());
 ...
 * @param pos
 */
void MapView2Widget::mousePos(QPoint pos)
{
