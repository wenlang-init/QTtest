#include "mapdownloadwidget.h"
#include "ui_mapdownloadwidget.h"

#include <QDateTime>
#include <QFileDialog>

MapDownLoadWidget::MapDownLoadWidget(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::MapDownLoadWidget)
{
    ui->setupUi(this);
    this->resize(600, 700);
    qApp->setStyleSheet("*{font: 9pt '宋体';}");
    this->setWindowTitle(QString("QT下载瓦片地图简单示例--V%1").arg(1.0));
    ...
    connect(m_dThread,
            ...
            this,
            &MapDownLoadWidget::finished);
    connect(m_dThreads,
            &DownloadThreads::finished,
            this,
            &MapDownLoadWidget::finished);
    ui->textEdit->document()->setMaximumBlockCount(100); // 最大显示行数
    ui->textEdit->moveCursor(QTextCursor::End);          // 自动滚动到底部

    qRegisterMetaType<QList<ImageInfo1> >("QList<ImageInfo1>");
    qRegisterMetaType<ImageInfo1>(        "ImageInfo1");
}

MapDownLoadWidget::~MapDownLoadWidget()
{
    delete m_dThread;
    delete m_dThreads;
    delete ui;
 ...
 * @brief 获取下载路径
 */
void MapDownLoadWidget::on_but_savePath_clicked()
{
 ...
 * @param checked
 */
void MapDownLoadWidget::on_but_thread_clicked(bool checked)
{
    if (checked)
    {
        m_timer.start();
        ...
        const QList<ImageInfo1>& infos = ui->mapInput->getInputInfo();
 ...
 * @param checked
 */
void MapDownLoadWidget::on_but_threads_clicked(bool checked)
{
    if (checked)
    {
        m_timer.start();
 ...
 * @param index  已经下载的索引
 */
void MapDownLoadWidget::finished(ImageInfo1 info)
{
