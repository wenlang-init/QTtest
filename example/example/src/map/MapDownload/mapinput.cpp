 ...
 * @return
 */
const QList<ImageInfo1>& MapInput::getInputInfo()
{
...
{
    static QString url =
        "https://server.arcgisonline.com/arcgis/rest/services/%1/MapServer/tile/%2/%3/%4.%5";
    ...
    for (int i = 0; i < count; i++)
    {
        if (count == 2)
        {
            ...
            else
            {
                ...
                info.format = "png";                                               //
                                                                                   //
                                                                                   // 如果同时下载卫星图和路网图则路网图为透明png格式
            }
        }
        QString tempUrl = url.arg(ui->com_amapPrefix->currentText());              //
                                                                                   //
                                                                                   // 设置域名
        tempUrl += QString("&style=%1").arg(style);                                //
                                                                                   //
                                                                                   // 设置地图类型
        tempUrl += QString("&lang=%1").arg(
            ui->com_amapLang->currentData().toString());                           //
                                                                                   //
                                                                                   // 设置语言
        tempUrl += QString("&scl=%1").arg(ui->com_amapScl->currentData().toInt()); //
                                                                                   //
                                                                                   // 设置图片尺寸，只在7
                                                                                   //
                                                                                   // 8生效
        tempUrl += QString("&ltype=%1").arg(ui->spin_amapLtype->value());          //
                                                                                   //
                                                                                   // 设置图片中的信息，只有
                                                                                   //
                                                                                   // 7
        ...
        for (int x = ltX; x <= rdX; x++)
        {
            ...
            for (int y = ltY; y <= rdY; y++)
            {
                info.url = tempUrl + QString("&x=%1&y=%2&z=%3").arg(x).arg(y).arg(
                    z);
                info.y = y;
                m_infos.append(info);
            }
...
#define USE_URL 1
    ...
    // https://r1.tiles.ditu.live.com/tiles/r1321001.png?g=1001&mkt=zh-cn
    static QString url =
        "https://r%1.tiles.ditu.live.com/tiles/%2%3.%4?g=1001&mkt=%5"; // 街道图r支持中文
#elif (USE_URL == 1)

    // http://dynamic.t2.tiles.ditu.live.com/comp/ch/r1321001.png?it=G,OS,L&mkt=en-us&cstl=w4c&ur=cn
    static QString url =
        "http://dynamic.t%1.tiles.ditu.live.com/comp/ch/%2%3.%4?it=G,OS,L&mkt=%5&cstl=%6&ur=cn";
#endif // if (USE_URL == 0)
...
#if (USE_URL == 0)
            info.url =
                url.arg(prefix).arg(type).arg(quadKey).arg(format).arg(lang);
#elif (USE_URL == 1)
            info.url =
                url.arg(prefix).arg(type).arg(quadKey).arg(format).arg(lang).arg(
                    cstl);
#endif // if (USE_URL == 0)
            m_infos.append(info);
        }
