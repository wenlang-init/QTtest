...
void httpGet(ImageInfo info)
{
    ...
    QSharedPointer<QNetworkReply> reply(manager.get(QNetworkRequest(QUrl(
                                                                        info.url))));
    ...
    if (reply->error() == QNetworkReply::NoError)
    {
        ...
        if (!buf.isEmpty())
        {
            ...
            if (!info.img.isNull())
            {
                ...
                emit GetUrlInterface::getInterface()->updateTitle(info.x,
                                                                  info.y,
                                                                  info.z);
                return;
            }
...
void GetUrl::getUrl()
{
    if (m_url.contains("{x}")) // XYZ格式
    {
        ...
        {
            m_infos[i].url = url.arg(m_infos[i].x).arg(m_infos[i].y).arg(
                m_infos[i].z);
        }
    }
