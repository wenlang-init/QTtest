...
// #define      MAX_MATRIX_SIZE                4194304             // 2048 *
// 2048
...
public:
    ...
    // 傅里叶转换 频域
    static bool fft1(QVector<Complex>  inVec,
                     int  const        len,
                     QVector<Complex>& outVec);

    // 逆转换
    static bool ifft(Complex  const inVec[],
                     int  const     len,
                     Complex        outVec[]);


    static int calMeanData(QList<double>  data,
                           QList<double>& retData,
                           double         fs);
     ...
     * fs:采样数:2^fs
     * */
    static void timeDomainToFreDomain(const CurveData& timeData,
