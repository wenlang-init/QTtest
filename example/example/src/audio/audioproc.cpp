...
void AudioProc::sqrt(CurveData& frequencyDomainData)
{
    ...
    while (left < right) {
        for (int i = left; i < right; i++) { // 保证 a[right] 是最大的
            if (frequencyDomainData.valVec[i] >
                frequencyDomainData.valVec[i + 1]) {
...
void AudioProc::processData(QByteArray& data, const QAudioFormat& format)
{
    ...
    case QAudioFormat::UInt8:
        ...
        for (qsizetype i = 0; i < data.size(); i++) {
            ...
            datasum += abs(timeData.valVec.last());
        }
        break;
    ...
    if (timeData.valVec.size() >
        0) datasum = datasum / (timeData.valVec.size() / 2);
...
# define MAXPOINTCNT 10
    ...
    for (int i = 0; i < frequencyDomainData.keyVec.size(); i++) {
        if ((max1.val < frequencyDomainData.valVec[i]) &&
            (frequencyDomainData.valVec[i] < max.val)) {
    ...
    for (int i = 0; i < frequencyDomainData.keyVec.size(); i++) {
        if ((max2.val < frequencyDomainData.valVec[i]) &&
            (frequencyDomainData.valVec[i] < max1.val)) {
    ...
    for (int i = frequencyDomainDatatmp.keyVec.size() - 1; i >= 0; i--) {
        ...
        if (frequencyDomainDatatmp.keyVec[i] != 0) str2 += "," + QString::number(
                2 / frequencyDomainDatatmp.keyVec[i],
                'g',
                16);
    }
    QString str = str1 + "\n" + str2;
