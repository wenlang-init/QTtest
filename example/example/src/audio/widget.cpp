    ...
    , ui(new Ui::Widget)
{
    ...
    connect(ui->pushButtonin,
            &QPushButton::clicked,
            this,
            &Widget::opensource);
    connect(ui->pushButtoninclose,
            &QPushButton::clicked,
            this,
            &Widget::closesource);
    connect(ui->pushButtonout, &QPushButton::clicked, this,
            &Widget::opensink);
    connect(ui->pushButtonoutclose,
            &QPushButton::clicked,
            this,
            &Widget::closesink);
    ...
    // connect(audiop,SIGNAL(dataflush(CurveData, QList<QPointF>,
    // QString)),this,SLOT(dataflush(CurveData, QList<QPointF>, QString)));
...
{
    connect(ui->comboBoxin, &QComboBox::currentTextChanged, this,
            [ = ](const QString& text) {
        for (int i = 0; i < QMediaDevices::audioInputs().size(); i++) {
            if (text == QMediaDevices::audioInputs().at(i).description()) {
                ui->comboBoxinbit->clear();
                QList<QAudioFormat::SampleFormat>sfmat = QMediaDevices::audioInputs().at(
                    i).supportedSampleFormats();
                ...
                ui->spinBoxin->setMaximum(QMediaDevices::audioInputs().at(i).
                                          maximumSampleRate());
                ui->spinBoxin->setMinimum(QMediaDevices::audioInputs().at(i).
                                          minimumSampleRate());

                ui->spinBoxinch->setMaximum(QMediaDevices::audioInputs().at(i).
                                            maximumChannelCount());
                ui->spinBoxinch->setMinimum(QMediaDevices::audioInputs().at(i).
                                            minimumChannelCount());

                ui->label_11->setText(QString("采样率(Hz):%1~%2").arg(ui->spinBoxin->
                                                                   minimum()).arg(
                                          ui->spinBoxin->maximum()));
                ui->label_12->setText(QString("通道数:%1~%2").arg(ui->spinBoxinch->
                                                               minimum()).arg(ui->
                                                                              spinBoxinch
                                                                              ->
                                                                              maximum()));

                switch (QMediaDevices::audioInputs().at(i).preferredFormat().
                        sampleFormat()) {
                case QAudioFormat::Unknown:
                ...
                switch (QMediaDevices::audioInputs().at(
                            i).channelConfiguration()) {
                ...
                case QAudioFormat::ChannelConfigSurround5Dot0:
                    ui->comboBoxinch->setCurrentText(
                ...
                ui->spinBoxin->setValue(QMediaDevices::audioInputs().at(i).
                                        preferredFormat().sampleRate());
                ui->spinBoxinch->setValue(QMediaDevices::audioInputs().at(i).
                                          preferredFormat().channelCount());
                break;
            }
        }
    });
    connect(ui->comboBoxout, &QComboBox::currentTextChanged, this,
            [ = ](const QString& text) {
        for (int i = 0; i < QMediaDevices::audioOutputs().size(); i++) {
            if (text == QMediaDevices::audioOutputs().at(i).description()) {
                ...
                QList<QAudioFormat::SampleFormat>sfmat = QMediaDevices::audioOutputs().at(
                    i).supportedSampleFormats();
                ...
                ui->spinBoxout->setMaximum(QMediaDevices::audioOutputs().at(i).
                                           maximumSampleRate());
                ui->spinBoxout->setMinimum(QMediaDevices::audioOutputs().at(i).
                                           minimumSampleRate());

                ui->spinBoxoutch->setMaximum(QMediaDevices::audioOutputs().at(i).
                                             maximumChannelCount());
                ui->spinBoxoutch->setMinimum(QMediaDevices::audioOutputs().at(i).
                                             minimumChannelCount());

                ui->label_3->setText(QString("采样率(Hz):%1~%2").arg(ui->spinBoxout->
                                                                  minimum()).arg(
                                         ui->spinBoxout->maximum()));
                ui->label_4->setText(QString("通道数:%1~%2").arg(ui->spinBoxoutch->
                                                              minimum()).arg(ui->
                                                                             spinBoxoutch
                                                                             ->
                                                                             maximum()));

                switch (QMediaDevices::audioOutputs().at(i).preferredFormat().
                        sampleFormat()) {
                case QAudioFormat::Unknown:
                ...
                switch (QMediaDevices::audioOutputs().at(
                            i).channelConfiguration()) {
                case QAudioFormat::ChannelConfigUnknown:
                ...
                case QAudioFormat::ChannelConfigSurround5Dot0:
                    ui->comboBoxoutch->setCurrentText(
                        "ChannelConfigSurround5Dot0");
                    break;
                ...
                ui->spinBoxout->setValue(QMediaDevices::audioOutputs().at(i).
                                         preferredFormat().sampleRate());
                ui->spinBoxoutch->setValue(QMediaDevices::audioOutputs().at(i).
                                           preferredFormat().channelCount());
...
void Widget::initcustomolot()
{
    ...
    // connect(customPlotfft->xAxis, SIGNAL(rangeChanged(QCPRange)),
    // customPlotfft->xAxis2, SLOT(setRange(QCPRange)));
    // connect(customPlotfft->yAxis, SIGNAL(rangeChanged(QCPRange)),
    // customPlotfft->yAxis2, SLOT(setRange(QCPRange)));
...
void Widget::handlePositionToolTip(QObject *watched, QMouseEvent *event)
{
    ...
    // 判断是否超出坐标轴范围
    if ((dValueX < m_pPlot->xAxis->range().lower) ||
        (dValueX > m_pPlot->xAxis->range().upper)
        || (dValueY < m_pPlot->yAxis->range().lower) ||
        (dValueY > m_pPlot->yAxis->range().upper)) return;
    ...
    for (int i = 0; i < m_vecGraph.count(); i++)
    {
        ...
        if (!m_vecGraph.at(i)->visible()) continue;
...
bool Widget::getGraphMinDistance(QCustomPlot *m_pPlot,
                                 QCPGraph    *m_vecGraph,
                                 int          iIndex,
                                 double     & dValueX,
                                 double     & dValueY)
{
    ...
    double dRatioX = m_pPlot->xAxis->axisRect()->width() /
                     (m_pPlot->xAxis->range().upper -
                      m_pPlot->xAxis->range().lower); // 求得X轴像素比
    double dRatioY = m_pPlot->yAxis->axisRect()->height() /
                     (m_pPlot->yAxis->range().upper -
                      m_pPlot->yAxis->range().lower); // 计算Y轴像素比

    double dx = fabs(x - dValueX);
    double dy = fabs(y - dValueY);

    //    qDebug()<<"index:"<<iIndex<<"=pointx:"<<QString::number(dValueX, 'f',
    // 8)<<",pointy:"<<QString::number(dValueY, 'f',
    // 8)<<"===x:"<<QString::number(x, 'f', 8)
    //             <<",y:"<<QString::number(y, 'f',
    // 8)<<"==dx:"<<QString::number(dx, 'f', 8)<<",dy:"<<QString::number(dy,
    // 'f', 8);
    // 判断是否在允许的范围内
...
void Widget::opensource()
{
    ...
    if (ui->comboBoxinbit->currentText() == "Unknown") {
        ...
        qDebug() << QString("not found format:%1").arg(
            ui->comboBoxinbit->currentText());
        return;
    }
    ...
    if (ui->comboBoxinch->currentText() == "ChannelConfigUnknown") {
        ...
        qDebug() << QString("not found ChannelConfig:%1").arg(
...
void Widget::opensink()
{
    ...
    connect(audioout, SIGNAL(stateChanged(QAudio::State)), this,
            SLOT(handleStateChanged(QAudio::State)));
    QIODevice *io = audioout->start();
    connect(this, &Widget::writeData, this,
            [ = ](QByteArray& data) {
        io->write(data);
    });
...
void Widget::handleStateChanged(QAudio::State newState)
{
    ...
    case QAudio::StoppedState:
        ...
        if (audioout == sender()) {
            ...
            DEBUG_PRINT_LOG("%s",
                            audioout->error() == QAudio::NoError ? "NoError" : audioout->error() == QAudio::OpenError ?
...
void Widget::dataflush(CurveData timeData,
                       ...
                       QString   str)
{
    ...
    customPlot->xAxis->setRange(index - 1 -
    ...
    for (int i = 0; i < frequencyDomainData.keyVec.size(); i++) {
        customPlotfft->graph(0)->addData(frequencyDomainData.keyVec[i],
                                         frequencyDomainData.valVec[i]);
    }
    customPlotfft->replot();
...
void Widget::on_pushButtonadd_clicked()
{
    ...
    ui->comboBox->addItem(QString("%1*cos(%2*x+%3)").arg(node.a).arg(node.b).arg(
                              node.c));
}
...
void Widget::timeout()
{
    ...
    for (int i = 0; i < pointcount; i++) {
        ...
        for (int coscnt = 0; coscnt < _cosparams.size(); coscnt++) {
            v += _cosparams[coscnt].a * cos(
                step * (i + 1) * _cosparams[coscnt].b + _cosparams[coscnt].c);
        }
        ...
        datasum += abs(v);
        _data.append((char *)&v, sizeof(v));
    }
    double db = 0;

    if (pointcount > 0) datasum = datasum / (pointcount / 2);

    if (datasum > 0) {
        db = 20 * log10(datasum);
    }

    emit writeData(_data);
    ...
    while (1) {
        ...
        for (int i = 0; i < frequencyDomainData.keyVec.size(); i++) {
            if (max[m].val < frequencyDomainData.valVec[i]) {
                if ((frequencyDomainData.valVec[i] <= max[m - 1].val) &&
                    (i != max[m - 1].index)) {
                    max[m].key = frequencyDomainData.keyVec[i];
                    max[m].val = frequencyDomainData.valVec[i];
                    max[m].index = i;
