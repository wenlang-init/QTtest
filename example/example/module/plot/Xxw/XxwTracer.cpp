    ...
    m_type(_type)
{
    ...
    if (m_plot)
    {
        ...
        case XAxisTracer:
        {
            ...
            m_label->position->setCoords(0, 20);
        ...
        case YAxisTracer:
        {
            ...
            m_arrow->end->setParentAnchor(m_tracer->position);
            m_arrow->start->setParentAnchor(m_arrow->end); // m_label->position);
...
XxwTracer::~XxwTracer()
{
    if (m_plot)
    {
        if (m_tracer) m_plot->removeItem(m_tracer);

        if (m_label) m_plot->removeItem(m_label);

        if (m_arrow) m_plot->removeItem(m_arrow);
    }
}

void XxwTracer::setPen(const QPen& pen)
{
    if (m_tracer) m_tracer->setPen(pen);

    if (m_arrow) m_arrow->setPen(pen);
}
...
void XxwTracer::setVisible(bool vis)
{
    ...
    if (m_tracer) m_tracer->setVisible(m_visible);

    if (m_label) m_label->setVisible(m_visible);

    if (m_arrow) m_arrow->setVisible(m_visible);
}
...
void XxwTracer::updatePosition(double xValue, double yValue)
{
    ...
    if (yValue >
        m_plot->yAxis->range().upper) yValue = m_plot->yAxis->range().upper;
