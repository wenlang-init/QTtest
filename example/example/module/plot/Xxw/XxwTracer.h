...
public:
    ...
    explicit XxwTracer(QCustomPlot *_plot,
                       TracerType   _type,
...
public:
    ...
    explicit XxwTraceLine(QCustomPlot *_plot,
                          LineType     _type = VerticalLine,
                          QObject     *parent = Q_NULLPTR);
    ~XxwTraceLine();
    void initLine();
    void updatePosition(double xValue,
    ...
    {
        if (m_lineV) m_lineV->setVisible(vis);

        if (m_lineH) m_lineH->setVisible(vis);
    }
