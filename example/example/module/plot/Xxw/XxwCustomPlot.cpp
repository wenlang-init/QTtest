...
void XxwCustomPlot::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (m_isShowTracer)
    {
        ...
        if (Q_NULLPTR == m_xTracer) m_xTracer = new XxwTracer(this,
        ...
        for (int i = 0; i < nGraphCount; ++i)
        {
            ...
            double dif_right =
                fabs(graph(i)->data()->at(index_right)->key - x_val);
            int iPointIdx =
                ((dif_left < dif_right) ? index_left : index_right);
            double x = graph(i)->data()->at(iPointIdx)->key;
            double y = graph(i)->data()->at(iPointIdx)->value;
            tracer->updatePosition(x, y);

            // 2个像素点内最近的x
            // double dRatioX = xAxis->axisRect()->width() /
            // (xAxis->range().upper - xAxis->range().lower); //求得X轴像素比
            ...
            //            double value =
            // this->graph(i)->data()->findBegin(x_val)->value;
            tracer->updatePosition(x_val, value);
#endif // if 1
        }

        if (Q_NULLPTR == m_lineTracer) m_lineTracer = new XxwTraceLine(this,
                                                                       XxwTraceLine::Both);
